...
- `plugins/QiwiTheme/`: Qiwi Theme companion plugin source for `thread-*` admin editing, Thread data storage, likes, IP location, external link stats, and the signed "own comments" cookie. Since 2.2.0 it also contains the merged `Sitemap.php` / `SitemapAction.php` (former `QiwiSitemap`: sitemap, robots.txt, timemachine RSS, feed discovery/enhancement) and `Mail.php` / `MailAction.php` / `MailConsole.php` / `page/mail.php` / `PHPMailer/` / `template/` / `sql/` (former `QiwiCommentMail`: queued comment mail with SMTP/Resend delivery and its admin console). All settings for these modules live in the theme config (`mail*` prefixed keys plus the sitemap keys), read via `QiwiTheme_Plugin::getThemeOption()` / `getThemeOptionArray()`; do not reintroduce plugin config forms for them.
...
- `plugins/QiwiTheme/` is the Qiwi theme companion plugin. When a feature cannot be solved reliably inside theme templates because it needs routes, actions, storage tables, admin APIs, or Typecho lifecycle hooks, put that logic in `QiwiTheme`. Since 2.2.0 the sitemap/feed module (former `QiwiSitemap`) and the comment mail module (former `QiwiCommentMail`) live inside it as self-contained module classes; new module code should follow that pattern (`QiwiTheme_Sitemap`, `TypechoPlugin\QiwiTheme\Mail`) with settings in the theme config.
- When changing companion plugin code for local testing, sync `plugins/*` into the local Typecho test runtime at `D:\phpstudy_pro\WWW\localhost\usr\plugins` before reporting completion. Use `scripts/sync-local-plugins.ps1`; it also refreshes selected plugin registrations so newly added hooks/actions/routes are written to Typecho's `options.plugins` record without deleting existing plugin configuration. `QiwiSitemap` and `QiwiCommentMail` no longer exist as standalone plugins; the sync script removes stale copies as obsolete.
- Typecho core deletes a plugin's `plugin:<Name>` option row when the plugin is deactivated, so re-enabling a plugin resets it to form defaults. Configurable companion plugins (`QiwiCap`) keep a `*_config_backup` option and implement a static `configHandle(array $settings, $isInit)` that restores the backup on activation and mirrors every save. Follow the same pattern when adding a new configurable plugin; do not call `Helper::configPlugin` from `activate()` because core overwrites it with defaults afterwards. Merged QiwiTheme modules avoid this entirely: their settings live in the theme config row, which survives plugin deactivation.
- When registering or deregistering plugins from inside `QiwiTheme_Plugin::activate()`, use the `Typecho_Plugin::exists()` / `Typecho_Plugin::deactivate()` API (in-memory registry) instead of editing the `plugins` option row directly — both the admin activation flow and `scripts/sync-local-plugins.ps1` re-export the in-memory registry afterwards and would overwrite direct row edits.
